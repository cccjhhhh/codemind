...
| Skill 路由系统重构 | ✅ 完成 | 2026-06-04 | 关键词+语义双重路由，置信度阈值，否定关键词检测 |
| CLI 交互增强 | ✅ 完成 | 2026-06-04 | 思考指示器、进度显示、环境变量支持（NO_COLOR 等） |
| DocGenSkill 意图识别 | ✅ 完成 | 2026-06-04 | 区分"生成内容"和"文档化代码" |
| OCP 合规改造 | ✅ 完成 | 2026-06-04 | Skill/Tool 扩展不需修改 CLI/AgentLoop |

### 2.1 重大更新记录（2026-06-04）

#### 更新 1: Skill 路由系统重构

**问题**：LLM 不主动使用 Skill，倾向直接调用 Tool

**解决方案**：采用 Claude Code 风格的显式路由

```
三层路由架构：
┌─────────────────────────────────────────────────────────────┐
│  Tier 1: 否定指标检查（最高优先级）                           │
│    → 包含"看看"、"聊聊"等词 → 直接返回 null，不触发任何 Skill │
├─────────────────────────────────────────────────────────────┤
│  Tier 2: 关键词匹配（确定性）                                 │
│    → 命中 SKILL.md 中的 triggerKeywords → 置信度 1.0         │
├─────────────────────────────────────────────────────────────┤
│  Tier 3: 语义路由（LLM 判断意图）                            │
│    → LLM 判断用户意图 + 置信度                               │
│    → confidence >= 0.7 → 触发 Skill                         │
│    → confidence < 0.7 → fallback 到普通 Chat                │
└─────────────────────────────────────────────────────────────┘
```

**新增文件**：
- `KeywordSkillRouter.java` - 关键词硬路由
- `SemanticSkillRouter.java` - 语义路由（带 LLM 判断）
- `SkillRoute.java` - 新增 confidence 字段和 shouldExecute() 方法
- `RouteReason.java` - 新增 LLM_INTENT_HIGH/LOW, NEGATIVE_INDICATOR

**参考**：Claude Code invocation control, LangChain confidence threshold

#### 更新 2: CLI 交互增强

**问题**：CLI 输出体验不如主流 Agent

**解决方案**：

| 功能 | 实现 | 参考 |
|------|------|------|
| 思考指示器 | `formatThinkingStart/End()` 在 LLM 生成响应前显示 | Claude Code, Cursor |
| 进度显示 | `formatProgress()` 显示 iteration + elapsed time | LangChain AgentExecutor |
| 环境变量 | NO_COLOR, CLICOLOR, CLICOLOR_FORCE 支持 | no-color.org 标准 |
| Skill 结果截断 | 超过 3 行/200 字符显示摘要 | Aider, Cursor |

#### 更新 3: DocGenSkill 意图识别

**问题**：用户说"生成文档呢，里面写一个诗歌"被当成文件路径
...
private boolean isGenerateContentIntent(String input) {
    // 生成内容的关键词：写一首、写个诗歌、创作等
}

// 判断是否自然语言句子（而非文件路径）
private boolean isNaturalLanguageSentence(String input) {
    // 包含中文标点、多个中文字符且无路径分隔符
}
```

#### 更新 4: OCP 合规改造

**目标**：新增 Skill/Tool 不需要修改 CLI/AgentLoop

**实现**：
- Skill 路由器通过构造函数注入
- OutputFormatter 使用 default 方法，新方法无需修改已有实现
- Skill 定义通过 SKILL.md 文件化，运行时可动态加载
...
> 
> **重要更新（2026-06-04）**：Skill 现在使用显式路由系统，通过关键词+语义双重路由触发，不再依赖 LLM 推理。

#### 4.5.0 Skill 架构设计（新）

**设计原则**：显式路由，确定性触发（参考 Claude Code）

```
┌─────────────────────────────────────────────────────────────┐
│                  Skill 路由架构（2026-06-04）                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  用户输入："帮我审查代码"                                     │
│      │                                                      │
│      ▼                                                      │
│  SkillRouter.route(input)                                   │
│      │                                                      │
│      ├─ Tier 1: 否定指标检查                                 │
│      │    → "看看"、"聊聊"等词？                             │
│      │    → 是 → return null（不触发）                       │
│      │                                                      │
│      ├─ Tier 2: 关键词匹配（KeywordSkillRouter）              │
│      │    → 匹配 SKILL.md 的 triggerKeywords                 │
│      │    → 命中 → return SkillRoute(confidence=1.0)         │
│      │                                                      │
│      └─ Tier 3: 语义路由（SemanticSkillRouter）              │
│           → LLM 判断用户意图                                 │
│           → confidence >= 0.7 → 触发 Skill                   │
│           → confidence < 0.7 → fallback 到 Chat              │
│                                                             │
│  SkillRoute.shouldExecute()                                 │
│      │                                                      │
│      ▼                                                      │
│  SkillDefinition.execute(context)                           │
│      │                                                      │
│      ▼                                                      │
│  SkillExecutor.execute(context)                             │
│      │                                                      │
│      ├── 内部调用 Tool: execute_command                      │
│      └── 内部调用 Tool: read_file                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| 组件 | 职责 | 文件 |
|------|------|------|
| **SkillRouter** | 路由总入口 | `SkillRouter.java`（接口）|
| **KeywordSkillRouter** | 关键词硬路由 | `KeywordSkillRouter.java` |
| **SemanticSkillRouter** | LLM 语义判断 | `SemanticSkillRouter.java` |
| **SkillRoute** | 路由结果 + 置信度 | `SkillRoute.java` |
| **RouteReason** | 路由原因追踪 | `RouteReason.java` |
...
详细设计见：[Skill 系统重新设计](./2026-06-04-skill-system-redesign.md)

#### 4.5.0.1 旧架构（已废弃）

> ⚠️ 以下设计已于 2026-06-04 废弃，保留作为历史参考

**旧设计原则**：Skill 包装成 Tool，供 LLM 调用（参考 LangChain）
...
**废弃原因**：LLM 不主动使用 Skill，倾向直接调用 Tool

详细设计见：[Skill 系统架构设计（旧）](./2026-06-03-skill-architecture-design.md)
